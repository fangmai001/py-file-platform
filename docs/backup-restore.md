...
**定時抓取由應用程式自己負責，不需要在主機上設定 cron。** 到管理後台的「RSS 訂閱」分頁，最上方的
「抓取排程」卡片有兩個設定：

- **啟用定時抓取**——預設關閉。勾起來之後平台就會自己按間隔抓取所有啟用中的來源。
- **間隔（分鐘）**——5 到 1440（一天）之間。存檔後立即生效，不必重啟服務。

同一張卡片上還有「全部立即抓取」按鈕，以及上一次整批執行的時間與結果摘要（例如
「12 個來源：成功 10、無更新 1、失敗 1，新增 5 則」）。單一來源各自的狀態仍顯示在下方列表的「狀態」欄。
...
先前的做法是把 `scripts/fetch-feeds.sh` 加進部署主機的 crontab。**啟用內建排程之後，請把 crontab 裡
那一行移除**：

```bash
crontab -e        # 刪掉 .../scripts/fetch-feeds.sh 那一行
```

兩邊同時開著不會產生重複文章（去重鍵是「來源 + 項目 guid」），只是白白多打對方站台一次而已。

內建排程預設關閉，正是為了讓既有部署升級之後行為不變——crontab 繼續照舊跑，什麼時候切換由你決定。

### 手動抓取（排錯用）

`scripts/fetch-feeds.sh` 仍然保留，適合在沒有瀏覽器的離線主機上排錯：它會逐一印出每個來源的結果，
結束碼也直接反映成敗。
...
它在 `app` container 內執行 `python -m app.cli.fetch_feeds`。走 container 內的 CLI 而不是打 HTTP API，
是因為抓取端點僅限管理員，從主機呼叫就得在那裡保管一份 JWT。

真的需要停用內建排程（例如想繼續用 cron 管理），在 `.env` 設定 `FEED_SCHEDULER_ENABLED=false`
再重啟 `app`。
